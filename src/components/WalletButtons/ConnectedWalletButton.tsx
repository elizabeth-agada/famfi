...
import { EXPLORER_URL } from '../../lib/stacks-api';
    ...
    <div className="">
      <Dialog>
        <DialogTrigger asChild>
          <button className="btn spray-dark px-10">{address ? shortenAddress(address) : "Not Connected"}</button>
        </DialogTrigger>

        <DialogContent className="max-w-sm w-full bg-white border-0">
          <DialogHeader>
          ...
          <div className="py-4 space-y-4">
            {/* Network */}
            <div className="py-3 border-2 border-spray-600 rounded px-4">
              ...
              <div className="relative flex items-center justify-between">
                <div className="">
                  <Wallet2 className="inline mr-2.5" size={20} />

                  <span className="font-medium inline-block">{address ? shortenAddress(address) : "Not Connected"}</span>
                </div>

                {/* Button to disconnect */}
                <Button className="btn chestnut px-5 py-2.5" onClick={disconnectWallet}> Disconnect </Button>
              </div>
            </div>

            {/* Wallet */}
            <section className="flex items-center justify-between text-sm w-full">
              <a className="space-x-1 flex items-center" target="_blank" href={`${EXPLORER_URL}address/${address}?chain=testnet`}>
                <span className="">View in explorer</span>
                <svg
                  className=" inline ml-2 fill-black" width="15" height="15" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg"
                >
